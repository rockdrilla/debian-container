...
package/essentials/bin/apt-sources -d ${distro} -s ${suite} "${uri}" > "${sources_tmp}"
...
mmdebstrap \
  ...
  --customize-hook="sync-in '${rootdir}/artifacts/${distro}-${suite}-essentials/pkg' /usr/local/bootstrap" \
