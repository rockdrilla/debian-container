...
build_single() {
	...
	GOLANG_BUILD_IMAGE="${IMAGE_PATH}/golang-build:${GOLANG_VERSION}-${SUITE}"

	set -e

	BUILD_IMAGE_TARGET=build \
	BUILD_IMAGE_PUSH=0 \
	scripts/build-image.sh image/golang/ "${GOLANG_BUILD_IMAGE}"

	podman image rm -f "${GOLANG_BUILD_IMAGE}"
